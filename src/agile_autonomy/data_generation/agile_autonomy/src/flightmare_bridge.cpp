...
FlightmareBridge::FlightmareBridge(const ros::NodeHandle& nh,
  ...
  RPGQ::QuadrotorID quad2_ID = 2;
  std::string quad_name = RPGQ::QuadrotorName(quad_ID);
  std::string quad2_name = RPGQ::QuadrotorName(quad2_ID);
  ...
  quad2_stereo_ = std::make_shared<RPGQ::Simulator::QuadStereoRGBCamera>(
      quad2_name, nullptr, 1000000);
  ...
  //设定2号机的左右目
  left_rgb_cam2_ = quad2_stereo_->GetLeftRGBCamera();
  left_rgb_cam2_->EnableOpticalFlow(false);
  left_rgb_cam2_->EnableCategorySegment(false);
  left_rgb_cam2_->EnableDepth(true);
  left_rgb_cam2_->SetWidth(img_cols_);
  left_rgb_cam2_->SetHeight(img_rows_);
  left_rgb_cam2_->SetFov(flightmare_fov);

  right_rgb_cam2_ = quad2_stereo_->GetRightRGBCamera();
  right_rgb_cam2_->EnableOpticalFlow(false);
  right_rgb_cam2_->EnableCategorySegment(false);
  right_rgb_cam2_->EnableDepth(false);
  right_rgb_cam2_->SetWidth(img_cols_);
  right_rgb_cam2_->SetHeight(img_rows_);
  right_rgb_cam2_->SetFov(flightmare_fov);

  // set the relative position of the camera with respect to quadrotor center 设定相机相对于机体中央的漂移
  // mass
  Eigen::Vector3d B_r_BCr(0.0, 0.0, 0.1);
  Eigen::Vector3d B_r_BCl(0.0, stereo_baseline_, 0.1);
  // set the relative rotation of the camera 设定相机的相对旋转
  ...
  right_rgb_cam2_->SetRelPose(B_r_BCr, R_BCr);
  left_rgb_cam2_->SetRelPose(B_r_BCl, R_BCl);
  // configure the quadrotor
  /**
   * @brief add the quad2 to show in flightmare & changed by chenlin in 20220119
   * 
   */
  quad_ = quad_stereo_->GetQuad();
  quad2_ = quad2_stereo_->GetQuad();
  Eigen::Vector3d quad_position{0.0, 0.0, 3.0};
  Eigen::Vector3d quad2_position{1.0, 0.0, 3.0};
  quad_->SetPos(quad_position);
  quad2_->SetPos(quad2_position);
  quad_->SetQuat(Eigen::Quaterniond(std::cos(0.5 * M_PI_2), 0.0, 0.0,
                                    std::sin(0.5 * M_PI_2)));
  quad2_->SetQuat(Eigen::Quaterniond(std::cos(0.5 * M_PI_2), 0.0, 0.0,
                                     std::sin(0.5 * M_PI_2)));
  // quad_->SetSize(Eigen::Vector3d(0.01, 0.01, 0.01));
  /**
   * @brief set the quad_size to bigger,in order to show clearly in flightmare & changed by chenlin in 20220119
   * 
   */
  quad_->SetSize(Eigen::Vector3d(0.5, 0.5, 0.5));
  quad2_->SetSize(Eigen::Vector3d(0.5, 0.5, 0.5));
  scene_id_ = env_idx_;

  // create flightmare birdge and connect sockets..
  // 陈林修改：增加flightmareBridge_ptr2_
  flightmareBridge_ptr_ = RPGQ::Simulator::FlightmareBridge::getInstance();
  flightmareBridge_ptr2_ = RPGQ::Simulator::FlightmareBridge::getInstance();
  flightmareBridge_ptr_->initializeConnections();
  //加了下面这句貌似会报错,可能这就是导致zmq冲突的来源
  // flightmareBridge_ptr2_->initializeConnections();

  flightmareBridge_ptr_->addQuadStereoRGB(quad_stereo_);
  flightmareBridge_ptr2_->addQuadStereoRGB(quad2_stereo_);
  ...
  // Unity is frozen as long as point cloud is created 直到点云生成好之前unity是冻结的
  ...
  unity_rgb_pub2_ = it.advertise("/hummingbird2/agile_autonomy2/unity_rgb2", 1);
  sgm_depth_pub2_ = it.advertise("/hummingbird2/agile_autonomy2/sgm_depth", 1);
...
void FlightmareBridge::getImageFromUnity2(
    const quadrotor_common::QuadStateEstimate& state_estimate,
    cv::Mat* left_frame, cv::Mat* right_frame, cv::Mat* gt_depth_frame) {
  // 1) set Pose camera in unity
  /**
   * @brief quad2 follow the quad1:pos + delta,copy the orientation && changed by chenlin in 20220119
   * 2号机跟随1号机的位置+偏差 姿态复制1号机
   * 
   */
  // Eigen::Vector3d delta = {2,0,0};
  Eigen::Vector3d quad_pos2 = state_estimate.position ;
  quad2_->SetPos(quad_pos2);
  quad2_->SetQuat(state_estimate.orientation);
  ...
  // 4) Receive frame
  left_rgb_cam2_->GetRGBImage(*left_frame);
  left_rgb_cam2_->GetDepthmap(*gt_depth_frame);
  right_rgb_cam2_->GetRGBImage(*right_frame);
}
...
void FlightmareBridge::getImages2(
    const quadrotor_common::QuadStateEstimate& state_estimate,
    const std::string curr_data_dir, const int frame_counter) {
  //2号机左右目图像
  cv::Mat left_rgb_img_2;
  cv::Mat right_rgb_img_2;

  cv::Mat gt_depth_img;
  cv::Mat gt_depth_uint16(img_rows_, img_cols_, CV_16UC1);
  printf("成功运行到1处"); 
  //给2号机的左右目图像赋值
  getImageFromUnity2(state_estimate, &left_rgb_img_2, &right_rgb_img_2,
                    &gt_depth_img);
  printf("成功运行到2处"); 
  std::string ty = type2str(gt_depth_img.type());
  for (int r = 0; r < img_rows_; ++r) {
  ...
  // compute disparity image
  //2号机深度图
  cv::Mat depth_uint16_2(img_rows_, img_cols_, CV_16UC1);
  computeDepthImage(left_rgb_img_2, right_rgb_img_2, &depth_uint16_2);
  printf("成功运行到3处"); 
  // overlay the speed on the left image for debugging / visualization
  cv::Mat rviz_rgb_img2 = left_rgb_img_2.clone();
  double speed = state_estimate.velocity.norm();
  std::stringstream stream;
  stream << std::fixed << "Speed: " << std::setprecision(2) << speed;
  std::string speed_string = stream.str();
  cv::putText(rviz_rgb_img2, speed_string, cv::Point(20, 20),  // Coordinates
              cv::FONT_HERSHEY_COMPLEX_SMALL,                 // Font
              1.0,                        // Scale. 2.0 = 2x bigger
              cv::Scalar(255, 255, 255),  // BGR Color
              1);                         // Line Thickness (Optional)
  ...
  // publish to rviz
  //发布2号机的unity_rgb图像 用的是2号机的左目图像
  sensor_msgs::ImagePtr rgb_msg2 =
      cv_bridge::CvImage(std_msgs::Header(), "bgr8", rviz_rgb_img2).toImageMsg();
  rgb_msg2->header.stamp = state_estimate.timestamp;
  unity_rgb_pub2_.publish(rgb_msg2);
  ...
  //发布2号机的sgm_depth图像 暂时和sgm_depth一样
  sensor_msgs::ImagePtr sgm_depth_msg2 =
      cv_bridge::CvImage(std_msgs::Header(), "mono16", depth_uint16_2)
          .toImageMsg();
  sgm_depth_msg2->header.stamp = state_estimate.timestamp;
  sgm_depth_pub2_.publish(sgm_depth_msg2);
  ...
  if (!curr_data_dir.empty()) {
    ...
    // Unity images
    // 5) Save frame
    cv::imwrite(left_rgb_img_filename, left_rgb_img_2);
    cv::imwrite(right_rgb_img_filename, right_rgb_img_2);
    cv::imwrite(depth_img_filename, depth_uint16_2);
    cv::imwrite(gt_depth_img_filename, gt_depth_uint16);
  }
}
