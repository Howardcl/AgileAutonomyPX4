...
AgileAutonomy::AgileAutonomy(const ros::NodeHandle& nh,
  ...
  //使用nh句柄，在话题前加命名空间/hummingbird/
  //使用pnh句柄，在话题前加命名空间和节点名 /hummingbird/agile_autonomy/
  // Subscribers
  ...
  ref_traj_pub_ =
      nh_.advertise<quadrotor_msgs::Trajectory>("/hummingbird2/agile_autonomy2/reference_trajectory", 1);
...
void AgileAutonomy::computeManeuver(const bool only_expert) {
  ...
  if (success) {
    // create directory 通过这条语句创建了目录名（带当前时间为后缀)
...
void AgileAutonomy::saveLoop(const ros::TimerEvent& time) {
  ...
  if (unity_is_ready_ && received_state_est_.isValid()) {
    ...
    {
      //这是个临时对象，当离开这段作用域时，自动析构，释放掉锁。无法取得锁的其他线程则会被阻塞。只有取得锁才能执行后续的操作。
...
void AgileAutonomy::odometryCallback(const nav_msgs::OdometryConstPtr& msg) {
  ...
  if (!acrobatic_trajectory_.points.empty()) {
    ...
    //如果状态机处在kExecteExpert或kNetwork模式下，则创建控制命令，然后发布控制命令。
  if (state_machine_ == StateMachine::kExecuteExpert ||
      state_machine_ == StateMachine::kNetwork) {
    ...
    //为2号机增加发布2022_03_01
    ref_traj_pub_.publish(reference_trajectory.toRosMessage());
...
void AgileAutonomy::completedGlobalPlanCallback(
  ...
  for (int i = 0; i < 1; i++) {
    std_msgs::Bool bool_msg;
    bool_msg.data = true;
    start_flying_pub_.publish(bool_msg);//这里参数为true,在stopflyingCallback()中不会进入切换到KComputeLabels模式
    ros::Duration(0.05).sleep();
  }
}
...
void AgileAutonomy::publishControlCommand(
  ...
  if (state_machine_ == StateMachine::kExecuteExpert ||
    ...
    //这里用的是1号机的state_predictor_,应该加一个uav_id判断用哪号无人机的状态估计器
