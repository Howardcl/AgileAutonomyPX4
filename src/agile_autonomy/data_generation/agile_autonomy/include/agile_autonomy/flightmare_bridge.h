...
class FlightmareBridge {
 public:
  ...
  void getImages2(const quadrotor_common::QuadStateEstimate& state_estimate,
                 const std::string curr_data_dir, const int frame_counter);
 private:
  ...
  //为2号机增加
  std::shared_ptr<RPGQ::Simulator::QuadrotorVehicle> quad2_;

  std::shared_ptr<RPGQ::Simulator::RGBCamera> left_rgb_cam_;
  std::shared_ptr<RPGQ::Simulator::RGBCamera> right_rgb_cam_;
  //为2号机增加
  std::shared_ptr<RPGQ::Simulator::RGBCamera> left_rgb_cam2_;
  std::shared_ptr<RPGQ::Simulator::RGBCamera> right_rgb_cam2_;

  std::shared_ptr<RPGQ::Simulator::FlightmareBridge> flightmareBridge_ptr_;
  //为2号机增加
  std::shared_ptr<RPGQ::Simulator::FlightmareBridge> flightmareBridge_ptr2_;
  ...
  //为2号机增加
  std::shared_ptr<RPGQ::Simulator::QuadStereoRGBCamera> quad2_stereo_;
  ...
  //为2号机增加
  image_transport::Publisher unity_rgb_pub2_;
  image_transport::Publisher unity_depth_pub2_;
  image_transport::Publisher sgm_depth_pub2_;
