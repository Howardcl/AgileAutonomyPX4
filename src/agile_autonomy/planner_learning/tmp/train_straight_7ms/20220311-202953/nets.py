...
def create_network(settings):
    net = PlaNet(settings)
    return net
#定义一个网络模型，模型名称为PLANET

class Network(Model):
    #自定义一个网络，继承Model
    def __init__(self):
        super(Network, self).__init__()
        #调用父类的初始化函数（该步为必须要的一步）
    ...
    def _create(self):
        ...
        # 具体实现方式：如果子类没有实现父类中指定要实现的方法，则会自动调用父类中的方法，而父类方法又是raise将错误抛出。
        # 这样代码编写者就能发现是缺少了对指定接口的实现。
...
class PlaNet(Network):
    ...
    def _plan_branch(self, embeddings):
        x = embeddings
        for f in self.plan_module:  # plan_module中是三层卷积层+一层全连接层
            x = f(x)
        return x 
    ...
    def _internal_call(self, inputs):
        ...
        imu_embeddings = self._imu_branch(imu_obs) #首先处理飞机的位姿和期望方向，对应B框图中的States Backbone
        img_embeddings = self._preprocess_frames(inputs) # 再处理输入的图像数据，对应B框图中的Depth Backbone
        ...
        output = self._plan_branch(total_embeddings) #网络计算，这个预测结果output应该是飞机轨迹（只包含位置）
        return output
